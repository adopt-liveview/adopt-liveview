...
We learned the basics of forms but we all know that a large part of the problem in forms revolves around validating data! Let's find out how LiveView handles these cases now.
...
We added the `phx-change="validate_product"` binding to our `<.form>` component so the `"validate_product"` event will be triggered whenever any input is modified. Furthermore, nothing in our `render/1` has been modified.

### The `<.input>` component

For errors to be displayed we need to define how they will be displayed. Inside our `Phoenix.Form.FormField` the `errors` property contains the list of errors in string format. A `div` with a `:for={msg <- @field.errors}` loop is enough. Since we are using a component, both of our fields automatically gain error validation!
...
Our keyword list of errors starts empty. If the value of `product_params["name"]` we use `Keyword.put/3` to add the error. The same is repeated for `description`.
...
Now open LiveView in your browser. Write anything in the name field and immediately see that the description field says it cannot be empty. We are currently not checking whether the field that was modified is the field being validated!

Furthermore, our LiveView has another problem. Leave the name field and write anything in the description field. The name disappeared?! What's going on here? Let's understand this now!

### How validation works

When you reassign the assign `form` in your `"validate_product`" event, LiveVIEW understands that all components that depend on it need an update. Furthermore, we update the current value of the form fields but we do not teach the component to use this updated value.
...
We swap our own definition of `<.input>` for the automatically generated definition. I hope that with the content so far you have understood the power behind this component, even if you never edit it.
...
- It is the responsibility of the `<.input>` component to render errors and to render the current value of the form field if it is modified on the server.
...
- Remember: in classes we will do `import SeuProjetoWeb.CoreComponents` however in real Phoenix projects this comes automatically when you do `use SeuProjetoWeb, :live_view`.
