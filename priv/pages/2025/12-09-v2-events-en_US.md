...
description: ~H"""
You must be wondering why in the callback <code>`mount/3`</code> we returned <code>`{:ok, socket}`</code> while in <code>`handle_event/3`</code> we used <code>`{:noreply, socket}`</code>. <br><br>The <code>`mount/3`</code> is just a function that LiveView executes while it's preparing its view, so it follows the Elixir pattern of saying "everything is OK, here's the initial socket". <br><br>But <code>`handle_event/3`</code> internally uses an Erlang/Elixir standard called <code>`GenServer`</code> ("Generic Server") and in the future we will see that we can also return a value for the element that generated the event with <code>`{:reply, map(), socket}`</code>!
"""
} %% .callout
